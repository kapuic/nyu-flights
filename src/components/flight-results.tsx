...
export function FlightResults({
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      {flights.map((flight) => (
        <Card
          className="rounded-[22px] border border-slate-200 bg-white shadow-none"
        ...
        >
          <CardHeader className="gap-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge
                className="rounded-full bg-blue-100 px-2.5 py-1 text-blue-700 hover:bg-blue-100"
                variant="secondary"
              >
                {flight.airlineName}
              </Badge>
              <Badge
                className="rounded-full border-slate-200 bg-slate-100 px-2.5 py-1 text-slate-700 hover:bg-slate-100"
                variant="outline"
              >
            ...
            <div className="grid gap-3 text-sm text-slate-600 sm:grid-cols-[1fr_auto_1fr] sm:items-center">
              ...
              <div className="text-center text-xs tracking-[0.18em] text-slate-400 uppercase">
                Direct
              </div>
          ...
          <CardContent className="flex flex-col gap-4">
              ...
              <div>
                ...
                <div className="mt-1 font-medium text-slate-950">
                  {flight.averageRating
                    ? `${flight.averageRating.toFixed(1)} / 5`
                    : "No ratings yet"}
                </div>
              </div>
            </div>
            {onChoose ? (
              <Button
                className="rounded-[14px] bg-slate-950 text-white hover:bg-slate-800"

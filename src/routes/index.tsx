  ...
  return (
    <TravelerShell currentUser={currentUser ? { displayName: currentUser.displayName, email: currentUser.email } : null} onLogout={handleLogout} section="explore">
      <div className="mx-auto w-full max-w-screen-2xl px-4 py-8 md:px-8 md:py-12">
        {/* Hero Search — matches Stitch traveler_flight_search_results */}
            ...
            <form className="relative z-20 flex w-full flex-col items-center gap-2 rounded-lg bg-white p-2 shadow-[0_20px_40px_-10px_rgba(25,28,30,0.08)] md:flex-row md:gap-4 md:p-4" onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); form.handleSubmit() }}>
              ...
              <div className="w-full flex-1 border-b-2 border-slate-100 px-2 pb-2 md:border-b-0 md:border-r-2 md:px-4 md:pb-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <form.Field name="source">{(field) => <input className="w-full border-none bg-transparent py-1 font-medium text-slate-950 placeholder-slate-400 focus:outline-none focus:ring-0" onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} placeholder="JFK - New York" type="text" value={field.state.value} />}</form.Field>
                </div>
              </div>

              {/* Swap Button */}
              <button className="relative z-30 -my-4 rounded-full bg-slate-100 p-2 text-slate-500 transition-colors hover:bg-slate-200 md:-mx-6 md:my-0" type="button">
              ...
              <div className="w-full flex-1 border-b-2 border-slate-100 px-2 pb-2 pt-4 md:border-b-0 md:border-r-2 md:px-4 md:pb-0 md:pt-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <form.Field name="destination">{(field) => <input className="w-full border-none bg-transparent py-1 font-medium text-slate-950 placeholder-slate-400 focus:outline-none focus:ring-0" onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} placeholder="LHR - London" type="text" value={field.state.value} />}</form.Field>
                </div>
              </div>
              ...
              <div className="w-full flex-1 px-2 pt-4 md:px-4 md:pt-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <form.Field name="departureDate">{(field) => <input className="w-full cursor-pointer border-none bg-transparent py-1 font-medium text-slate-950 focus:outline-none focus:ring-0" onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} type="date" value={field.state.value} />}</form.Field>
                </div>
              </div>
            ...
            <form.Field name="tripType">
              {(field) => (
                <div className="mt-6 flex flex-wrap items-center gap-4 text-center md:justify-start">
                  ...
                  <button className="flex items-center gap-1 text-sm font-medium text-slate-950 transition-colors hover:text-slate-600" type="button">
        ...
        {/* Results Area — editorial asymmetry, 12-col grid */}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="space-y-2">
                    <FilterCheckbox label="Nonstop" price="$450" defaultChecked />
                    <FilterCheckbox label="1 Stop" price="$380" />
                  </div>
                </div>
                <div>
                  <h4 className="mb-3 text-sm font-medium text-slate-950">Airlines</h4>
                  <div className="space-y-2">
                    <FilterCheckbox label="AeroPrecision" defaultChecked />
                    <FilterCheckbox label="Global Airways" defaultChecked />
                    <FilterCheckbox label="SkyNet Connect" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6 lg:col-span-9">
            <div className="mb-6 flex items-end justify-between">
              ...
              <div className="hidden items-center gap-2 md:flex">
                ...
                <button className="flex items-center gap-1 text-sm font-medium text-slate-950">
            ...
            {searchResults ? (
              ...
              <div className="space-y-6">
                ...
                <DemoFlightCard airline="Global Airways GA-44" arrivalCode="LHR" arrivalTime="08:55" departureCode="JFK" departureTime="21:00" duration="6h 55m" hasPlusOne price={520} seatsLeft={3} status={undefined} variant="secondary" />
              </div>
            )}
          </div>
        </section>
...
/* ─── Flight Card — Stitch traveler_flight_search_results card ─── */
...
function FlightCard({ currentUser, flight }: { currentUser: Awaited<ReturnType<typeof getCurrentUserFn>>; flight: FlightOption }) {
  ...
  const plusOne = arrDate > depDate
  const stops = "Nonstop"

  return (
    <article className="flex flex-col items-center gap-6 rounded-lg bg-white p-6 shadow-[0_4px_16px_rgba(0,0,0,0.02)] transition-transform duration-300 hover:-translate-y-1 md:flex-row">
      <div className="w-full flex-1 space-y-4">
        ...
        <div className="flex w-full items-center justify-between">
          ...
          <div className="flex flex-1 flex-col items-center px-8">
            ...
            <div className="mt-1 text-xs text-slate-500">{stops}</div>
          </div>
      ...
      <div className="flex w-full flex-row items-center justify-between md:w-auto md:flex-col md:items-end md:justify-center md:gap-4">
        ...
        <Link className="whitespace-nowrap rounded-lg bg-slate-950 px-6 py-2 font-medium text-white transition-opacity hover:opacity-90" to={currentUser?.role === "customer" ? "/customer" : "/login"}>Select</Link>
      </div>
    </article>
  )
}

/* ─── Demo Flight Card (pre-search state) ─── */
  ...
  return (
    <article className="flex flex-col items-center gap-6 rounded-lg bg-white p-6 shadow-[0_4px_16px_rgba(0,0,0,0.02)] transition-transform duration-300 hover:-translate-y-1 md:flex-row">
      <div className="w-full flex-1 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className={cn("flex size-6 items-center justify-center rounded-sm", isPrimary ? "bg-slate-950 text-white" : "bg-slate-200 text-slate-500")}>
              <Plane className="size-3" />
            </div>
            <span className="text-sm font-medium text-slate-950">{airline}</span>
          </div>
          {status ? (
            <Badge className="rounded-sm bg-[#cde5ff] px-2 py-1 text-[0.6875rem] font-bold uppercase tracking-[0.05em] text-[#004b74]" variant="secondary">{status}</Badge>
          ) : seatsLeft ? (
            <Badge className="flex items-center gap-1 rounded-sm bg-slate-200 px-2 py-1 text-[0.6875rem] font-bold uppercase tracking-[0.05em] text-slate-500" variant="secondary">
      ...
      <div className="flex w-full flex-row items-center justify-between md:w-auto md:flex-col md:items-end md:justify-center md:gap-4">
        ...
        <Link className={cn("whitespace-nowrap rounded-lg px-6 py-2 font-medium transition-colors", isPrimary ? "bg-slate-950 text-white hover:bg-slate-800" : "bg-slate-200 text-slate-950 hover:bg-slate-300")} to="/login">Select</Link>
      </div>
    </article>
  )
}

/* ─── Filter Checkbox — Stitch filter sidebar ─── */

function FilterCheckbox({ label, price, defaultChecked }: { label: string; price?: string; defaultChecked?: boolean }) {
  return (
    <label className="flex cursor-pointer items-center justify-between group">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm text-slate-500 transition-colors group-hover:text-slate-950">{label}</span>
      </div>
  ...
  return (
    <div className="rounded-lg bg-slate-50 p-10 text-center">
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-4">
        <Button className="rounded-lg bg-slate-200 text-slate-950 hover:bg-slate-300" variant="secondary">Clear Filters</Button>
        <Button className="rounded-lg bg-white text-slate-950 shadow-sm hover:bg-slate-50" variant="outline">Return Home</Button>
      </div>
    </div>
  )
}

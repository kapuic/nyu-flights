...
import { ArrowLeft, ArrowRight, CheckCircle2, Clock, CreditCard, Dices, Plane } from "lucide-react";
...
function RouteCodes({ flight }: { flight: FlightOption }) {
  return (
    <span className="inline-flex items-center gap-1.5">
      <CountryFlag countryCode={flight.departureCountryCode} size={14} />
      {flight.departureAirportCode}
      <span aria-hidden="true">→</span>
      {flight.arrivalAirportCode}
      <CountryFlag countryCode={flight.arrivalCountryCode} size={14} />
    </span>
  );
}
...
function CheckoutForm({
  ...
  const totalPrice = outbound.basePrice + (returnFlight ? returnFlight.basePrice : 0);
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
        <div>
          ...
          <div className="mb-6 space-y-3">
            ...
            {returnFlight ? <FlightSummaryCard flight={returnFlight} label="Return" /> : null}
          </div>
          ...
          <div className="rounded-xl border border-white/[0.08] bg-white/[0.04] p-6 backdrop-blur-xl">
            ...
            <form noValidate ref={formRef} onSubmit={handleSubmit}>
              ...
              >
                {submitting ? "Processing…" : `Confirm & Pay ${formatCurrency(totalPrice)}`}
              </button>
            </form>
          </div>
        </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-white/60">
                  <RouteCodes flight={outbound} />
                </span>
                <span className="text-white">{formatCurrency(outbound.basePrice)}</span>
              </div>
              {returnFlight ? (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-white/60">
                    <RouteCodes flight={returnFlight} />
                  </span>
                  <span className="text-white">{formatCurrency(returnFlight.basePrice)}</span>
                </div>
              ) : null}
            </div>
...
function FlightSummaryCard({ flight, label }: { flight: FlightOption; label: string }) {
  return (
...
function ConfirmationView({
  ...
  return (
    <main className="relative z-10 mx-auto flex max-w-2xl flex-col items-center px-4 pt-16 pb-24 text-center">
      ...
      <h1 className="mb-2 text-2xl font-semibold text-white">Booking Confirmed</h1>
      <p className="mb-8 text-sm text-white/40">
          ...
          >
            ...
            <div className="flex items-center gap-4">
              <div>
                ...
                <div className="text-xs text-white/40">{formatTime(flight.departureDatetime)}</div>
              </div>
              ...
              <div className="text-right">
                <div className="flex items-center justify-end gap-1.5 text-lg font-semibold text-white">
                  {flight.arrivalAirportCode}
                  <CountryFlag countryCode={flight.arrivalCountryCode} size={18} />
                </div>
                <div className="text-xs text-white/40">{formatTime(flight.arrivalDatetime)}</div>
              </div>
            </div>
